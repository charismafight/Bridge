...
REGSTR2 = r"qx\|(.+)\|pn\|(.+)\|md\|(\d)(.+)\|sv\|(\w)\|rh\|\|ah\|(Board\s\d+)\|mb\|(.+)\|mb\|p\|mb\|p\|mb\|p\|pg\|\|pc\|(.+)\|pg\|\|$"
    ...
    try:
        ...
        for s in info2:
            ...
            if re.match(REGSTR1,s):
                ...
                string3 = re.match(REGSTR2,s).group(4)
            ...
            if cards[3] != '':
                ecards_s = re.match(r"S(.*)H(.*)D(.*)C(.*)$",cards[3]).group(1)
                doc.Tables[tnum].Rows[4].Cells[1].Range.Text = u"\u2660 " + sortstr(ecards_s)
                ecards_h = re.match(r"S(.*)H(.*)D(.*)C(.*)$",cards[3]).group(2)
                doc.Tables[tnum].Rows[5].Cells[1].Range.Text = u"\u2665 " + sortstr(ecards_h)
                ecards_d = re.match(r"S(.*)H(.*)D(.*)C(.*)$",cards[3]).group(3)
                doc.Tables[tnum].Rows[6].Cells[1].Range.Text = u"\u2666 " + sortstr(ecards_d)
                ecards_c = re.match(r"S(.*)H(.*)D(.*)C(.*)$",cards[3]).group(4)
                doc.Tables[tnum].Rows[7].Cells[1].Range.Text = u"\u2663 " + sortstr(ecards_c)
            ...
            for bbb in bid:
                if re.match("(\d[H|C|D|S])(!?)\|an\|(.+)",bbb):
                    describe.append(re.match("(\d[H|C|D|S])(!?)\|an\|(.+)",bbb).group(1) + r": " + re.match("(\d[HCDSN])(!?)\|an\|(.+)",bbb).group(3))
                if re.match("(\dN)(!?)\|an\|(.+)",bbb):
                    describe.append(re.match("(\dN)(!?)\|an\|(.+)",bbb).group(1).replace("N","NT") + r": " + re.match("(\d[HCDSN])(!?)\|an\|(.+)",bbb).group(3))
            ...
            for i in range(len(bid)):
                ...
                if re.match("^\dN$",bid[i][0:2]):
                    doc.Tables[tnum].Rows[13 + int((i + flag1) / 4)].Cells[(i + flag1) % 4].Range.Text = bid[i][0:2].replace("N","NT")
...
def genWord():
    ...
    formdoc = cur_file_dir() + "\\" + r"form_practice.docx"
...
record = []
threads = []
t1 = threading.Thread(target=handlelin,args=(firstFile,))
t2 = threading.Thread(target=drawPic)
threads.append(t1)
threads.append(t2)
for t in threads:
    t.setDaemon(True)
